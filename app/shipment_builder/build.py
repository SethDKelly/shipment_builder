...
def summary(shipments):
    ...
    if 'package_id' in shipments.keys() :
        data['Total Shipments'] = len(shipments.package_id.unique())
        data['Cubic Volume not Utilized'] = (1.58*len(shipments.package_id.unique()) - shipments.cubic_volume_ft.sum())
        data['Percent Cubic Volume not Utilized'] = round(((1.58 * len(shipments.package_id.unique()) - shipments.cubic_volume_ft.sum()) / 
